...
class DeepseekMLAForwardMixin:
    ...
    def forward_absorb_prepare(
        ...
        # SGLang's generic CPU offloader can leave this derived MLA projection
        # tensor on CPU while the active shard tensors are already on CUDA.
        # Materialize only this operation's view on the live device.
        w_kc = (
            self.w_kc.to(q_nope.device, non_blocking=True)
            if self.w_kc.device != q_nope.device
            else self.w_kc
        )
        w_scale = (
            self.w_scale.to(q_nope.device, non_blocking=True)
            if hasattr(self.w_scale, "device") and self.w_scale.device != q_nope.device
            else self.w_scale
        )
    ...
    def forward_absorb_core(
        ...
        w_vc = (
            self.w_vc.to(attn_output.device, non_blocking=True)
            if self.w_vc.device != attn_output.device
            else self.w_vc
        )
        w_scale_out = (
            self.w_scale.to(attn_output.device, non_blocking=True)
            if hasattr(self.w_scale, "device")
            and self.w_scale.device != attn_output.device
            else self.w_scale
        )
        ...
        if self.use_deep_gemm_bmm:
            ...
            if _is_cpu:
                attn_bmm_output = torch.bmm(
                    attn_output.to(torch.bfloat16).transpose(0, 1),
                    w_vc.to(torch.bfloat16) * w_scale_out,
                )
                attn_bmm_output = attn_bmm_output.transpose(0, 1).flatten(1, 2)
            else:
