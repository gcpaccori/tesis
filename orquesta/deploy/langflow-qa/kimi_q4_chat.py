"""Kimi 2.6 SGLang Long Agent component.

This component avoids LangFlow provider dropdown issues by calling the local
OpenAI-compatible SGLang endpoint directly.
"""
...
DEFAULT_SYSTEM_PROMPT = """Eres Kimi 2.6 servido localmente por SGLang actuando como cerebro de agente largo para QA empresarial.
...
class KimiQ4LongAgent(Component):
    display_name = "Kimi K2.6 Q2 GPU Agent"
    description = "Chat directo con Kimi K2.6 Q2 local via llama.cpp OpenAI-compatible, preparado para agentes largos."
    icon = "BrainCircuit"
    ...
    inputs = [
        ...
        StrInput(
            ...
            value="http://qa-kimi-q2-gpu:8021/v1",
            required=True,
            advanced=True,
        ),
        StrInput(
    ...
    def run_agent(self) -> Message:
        ...
        if not user_text:
            message = Message(text="No llego mensaje para Kimi K2.6.")
            self.status = message
            return message
        ...
        if not text:
            text = "Kimi K2.6 respondio sin contenido visible. Sube max_tokens o desactiva thinking."
