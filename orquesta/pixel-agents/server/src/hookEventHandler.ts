...
export class HookEventHandler {
  ...
  private applySessionMetadata(
    ...
    const teamName = typeof event.team_name === 'string' ? event.team_name : undefined;
    const agentName = typeof event.agent_name === 'string' ? event.agent_name : undefined;
    const isTeamLead =
      typeof event.is_team_lead === 'boolean' ? event.is_team_lead : agent.isTeamLead;
  ...
  handleEvent(_providerId: string, event: HookEvent): void {
    ...
    if (normEvent.kind === 'sessionStart') {
      ...
      if (existingAgentId !== undefined) {
        ...
        this.applySessionMetadata(existingAgentId, event, this.getWebview());
        if (debug)
          console.log(
      ...
      for (const [id, agent] of this.agents) {
        if (agent.sessionId === event.session_id) {
          ...
          agent.hookDelivered = true;
          this.applySessionMetadata(id, event, this.getWebview());
          if (debug)
            console.log(
      ...
      if (transcriptPath || cwd) {
        ...
        this.pendingExternalSessions.set(event.session_id, {
          ...
          transcriptPath,
          cwd: cwd ?? '',
          teamName: typeof event.team_name === 'string' ? event.team_name : undefined,
          agentName: typeof event.agent_name === 'string' ? event.agent_name : undefined,
          isTeamLead: typeof event.is_team_lead === 'boolean' ? event.is_team_lead : undefined,
        });
      } else {
        if (debug && tracked)
          console.log(
    ...
    if (this.pendingExternalSessions.has(event.session_id)) {
      ...
      const adoptedAgent = [...this.agents.values()].find((a) => a.sessionId === pending.sessionId);
      if (adoptedAgent) {
        this.applySessionMetadata(
          adoptedAgent.id,
          {
            ...event,
            team_name: pending.teamName,
            agent_name: pending.agentName,
            is_team_lead: pending.isTeamLead,
          },
          this.getWebview(),
        );
      }
