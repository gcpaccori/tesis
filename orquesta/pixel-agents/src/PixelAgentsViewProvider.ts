...
import { getDefaultProvider } from '../server/src/providers/index.js';
...
export class PixelAgentsViewProvider implements vscode.WebviewViewProvider {
  private readonly activeProvider = getDefaultProvider();
  ...
  private dispatchHookEvent(event: HookEvent): void {
    this.hookEventHandler?.handleEvent(this.activeProvider.id, event);
  }

  private dispatchAgentSessionStart(agent: AgentState): void {
    this.dispatchHookEvent({
      hook_event_name: 'SessionStart',
      session_id: agent.sessionId,
      cwd: agent.projectDir,
      source: this.activeProvider.id,
      team_name: agent.teamName,
      agent_name: agent.agentName,
      is_team_lead: agent.isTeamLead,
    });
  }

  private clearAgentActivityViaHooks(agent: AgentState): void {
    this.dispatchHookEvent({
  ...
  private assignTaskToAgent(agentId: number, description: string): void {
    const agent = this.agents.get(agentId);
    if (!agent) return;
    const task = description.trim();
    if (!task) return;
    this.clearAgentActivityViaHooks(agent);
    this.dispatchAgentSessionStart(agent);
    this.dispatchHookEvent({
      hook_event_name: 'PreToolUse',
      session_id: agent.sessionId,
      tool_name: agent.isTeamLead ? 'Plan' : 'Think',
      tool_input: { description: task },
    });
  }

  private requestAgentPermission(agentId: number): void {
    const agent = this.agents.get(agentId);
    if (!agent) return;
    this.dispatchAgentSessionStart(agent);
    this.dispatchHookEvent({
  ...
  private markAgentIdle(agentId: number): void {
    const agent = this.agents.get(agentId);
    if (!agent) return;
    this.clearAgentActivityViaHooks(agent);
  }

  private closeOrquestaAgent(agentId: number): void {
    const agent = this.agents.get(agentId);
    if (!agent) return;
    this.dispatchHookEvent({
  ...
  private isRelevantExternalSession(projectDir: string, transcriptPath?: string): boolean {
    if (this.watchAllSessions.current) return true;
    if (transcriptPath) {
      return isTrackedProjectDir(projectDir);
    }

    const workspaceFolders = vscode.workspace.workspaceFolders ?? [];
    const normalizedProjectDir = path.resolve(projectDir).toLowerCase();
    return workspaceFolders.some((folder) => {
      const workspacePath = path.resolve(folder.uri.fsPath).toLowerCase();
      return (
        normalizedProjectDir === workspacePath ||
        normalizedProjectDir.startsWith(workspacePath + path.sep.toLowerCase())
      );
    });
  }

  private installActiveProviderHooks = async (): Promise<void> => {
    const config = this.pixelAgentsServer?.getConfig();
    if (!config) return;
    await this.activeProvider.prepareHooks?.(this.context.extensionPath);
    await this.activeProvider.installHooks(`http://127.0.0.1:${config.port}`, config.token);
  };
  ...
  private initHooks(): void {
    ...
    // Register the active provider's team provider (if present) with the file
    // watcher module + transcriptParser, plus the teammate removal callback.
    if (this.activeProvider.team) {
      setTeamProvider(this.activeProvider.team);
    }
    setHookProvider(this.activeProvider);
    ...
    this.hookEventHandler.setLifecycleCallbacks({
      onExternalSessionDetected: (sessionId, transcriptPath, cwd) => {
        ...
        if (!this.isRelevantExternalSession(projectDir, transcriptPath)) {
    ...
    this.pixelAgentsServer
      ...
      .then((config) => {
        ...
        if (hooksEnabled) {
          void this.installActiveProviderHooks().catch((e) => {
            console.error(`[Pixel Agents] Failed to prepare hooks: ${e}`);
          });
        }
        console.log(
          `[Pixel Agents] Server: ready on port ${config.port} for provider ${this.activeProvider.id}`,
        );
      })
      .catch((e) => {
  ...
  resolveWebviewView(webviewView: vscode.WebviewView) {
    ...
    webviewView.webview.onDidReceiveMessage(async (message) => {
      if (message.type === 'openClaude') {
        if (this.activeProvider.id !== 'claude') {
          void vscode.window.showInformationMessage(
            'Pixel Agents esta en modo Orquesta. Los agentes se crean por eventos, no por la terminal Claude.',
          );
          return;
        }
      ...
      } else if (message.type === 'orquestaAssignTask') {
        this.assignTaskToAgent(message.id as number, String(message.description ?? ''));
      } else if (message.type === 'orquestaPermission') {
        this.requestAgentPermission(message.id as number);
      } else if (message.type === 'orquestaIdle') {
        this.markAgentIdle(message.id as number);
      } else if (message.type === 'orquestaClose') {
        this.closeOrquestaAgent(message.id as number);
        ...
        if (enabled) {
          void this.installActiveProviderHooks().catch((e) => {
            console.error(`[Pixel Agents] Failed to enable hooks: ${e}`);
          });
          console.log(`[Pixel Agents] Hooks enabled for provider ${this.activeProvider.id}`);
        } else {
          void this.activeProvider.uninstallHooks().catch((e) => {
            console.error(`[Pixel Agents] Failed to disable hooks: ${e}`);
          });
          console.log(`[Pixel Agents] Hooks disabled for provider ${this.activeProvider.id}`);
        }
