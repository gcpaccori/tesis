...
import { initBrowserSwarm, seedBrowserSwarm } from './browserSwarm.js';
...
let mockMessagesDispatched = false;
...
async function decodeCharactersFromPng(
  ...
  for (const relPath of index.characters) {
    const png = await decodePng(
      buildAppUrl(base, `assets/${getIndexedAssetPath('characters', relPath)}`),
    );
...
async function decodeFloorsFromPng(base: string, index: AssetIndex): Promise<string[][][]> {
  ...
  for (const relPath of index.floors) {
    const png = await decodePng(
      buildAppUrl(base, `assets/${getIndexedAssetPath('floors', relPath)}`),
    );
...
async function decodeWallsFromPng(base: string, index: AssetIndex): Promise<string[][][][]> {
  ...
  for (const relPath of index.walls) {
    const png = await decodePng(
      buildAppUrl(base, `assets/${getIndexedAssetPath('walls', relPath)}`),
    );
    const set: string[][][] = [];
...
async function decodeFurnitureFromPng(
  ...
  for (const entry of catalog) {
    const png = await decodePng(buildAppUrl(base, `assets/${entry.furniturePath}`));
...
export async function initBrowserMock(): Promise<void> {
  ...
  await initBrowserSwarm(base);

  const [assetIndex, catalog] = await Promise.all([
    fetchJsonWithFallback<AssetIndex>(base, 'assets/asset-index.json'),
    fetchJsonWithFallback<CatalogEntry[]>(base, 'assets/furniture-catalog.json'),
  ]);
  ...
  const [decodedCharacters, decodedFloors, decodedWalls, decodedFurniture] = shouldTryDecoded
    ? await Promise.all([
        fetchJsonOptional<CharacterDirectionSprites[]>(
          buildAppUrl(base, 'assets/decoded/characters.json'),
        ),
        fetchJsonOptional<string[][][]>(buildAppUrl(base, 'assets/decoded/floors.json')),
        fetchJsonOptional<string[][][][]>(buildAppUrl(base, 'assets/decoded/walls.json')),
        fetchJsonOptional<Record<string, string[][]>>(
          buildAppUrl(base, 'assets/decoded/furniture.json'),
        ),
      ])
    : [null, null, null, null];
  ...
  const layout = assetIndex.defaultLayout
    ? await fetchJsonWithFallback<unknown>(base, `assets/${assetIndex.defaultLayout}`)
    : null;
...
export function dispatchMockMessages(): void {
  if (!mockPayload || mockMessagesDispatched) return;
  mockMessagesDispatched = true;
