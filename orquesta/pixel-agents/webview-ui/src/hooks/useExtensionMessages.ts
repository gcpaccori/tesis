...
export interface SwarmCellState {
  id: string;
  label: string;
  division: string;
  supervisor: string;
...
export function useExtensionMessages(
  ...
  const [chatMessages, setChatMessages] = useState<RoundtableMessage[]>([]);
  const [cellStates, setCellStates] = useState<Record<string, SwarmCellState>>({});
  ...
  useEffect(() => {
    ...
    const handler = (e: MessageEvent) => {
      ...
      if (msg.type === 'layoutLoaded') {
        ...
        os.removeAgent(id);
      } else if (msg.type === 'orquestaChatMessage') {
        const entry = msg.entry as RoundtableMessage;
        setChatMessages((prev) => [...prev.slice(-159), entry]);
      } else if (msg.type === 'orquestaCellState') {
        const cell = msg.cell as SwarmCellState;
        setCellStates((prev) => ({ ...prev, [cell.id]: cell }));
        ...
        if (typeof msg.providerId === 'string') {
          setProviderId(msg.providerId as string);
        }
